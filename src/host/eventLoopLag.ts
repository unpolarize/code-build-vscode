...
import { getHostTask } from './hostTrace';
...
export function formatLagLine(p50: number, p99: number, task?: string): string {
  const stall = p99 > LAG_STALL_MS ? ' STALL' : '';
  const t = task ? ` task=${task}` : '';
  return `[lag] p50=${p50.toFixed(1)}ms p99=${p99.toFixed(1)}ms${stall}${t}`;
}
...
export function startEventLoopLagMonitor(
  ...
  const timer = setInterval(() => {
    ...
    appendLine(formatLagLine(p50, p99, p99 > LAG_STALL_MS ? getHostTask() : undefined));
    hist.reset();
  }, intervalMs);
  timer.unref?.();
  return {
